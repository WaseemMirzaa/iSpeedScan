...
class _ScannerWidgetState extends State<ScannerWidget>
  ...
  Future<void> checkSubscriptionStatus() async {
    ...
    if (_customerInfo?.entitlements.all['sub_lifetime_ispeedscan']?.isActive ==
        true) {
      setState(() {
        _isSubscribed = true;
  ...
  Future<void> _loadMode() async {
    ...
    try {
      ...
      print('🟢🟢🟢🟢🟢 Subscription Exception Caught $e');
    }

    bool savedMode = await PreferenceService.getMode();
    String savedQuality = await PreferenceService.getPDFQuality();

    setState(() {
      isPhotoMode = savedMode;
      selectedValue = savedQuality;
    });
      ...
      try {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          if (receipt != null) {
            ...
            LogHelper.logMessage(
                'Receipt Data', '${receipt.substring(0, 50)}...');
            ...
            LogHelper.logMessage(
                'Apple Response', 'Status: ${responseData['status']}');
        ...
        LogHelper.logErrorMessage(
            'Receipt Status', 'Failed to get receipt after 3 attempts');
        return false;
      } catch (e) {
